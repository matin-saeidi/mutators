...
void population::mutateup(int n)
{
    ...
    for (int i=0; i<n; i++)
    {
        if ((alleleholders[0] != 0) &&
            (mt_rand(gent) > (0.5 * alleleholders[1]) / alleleholders[0]))
        {
            // Case 1: mutate a hom-ancestral → heterozygote
            alleleholders[0] -= 1;
            alleleholders[1] += 1;
        }
        else if (alleleholders[1] != 0)
        {
            // Case 2: mutate a heterozygote → hom-derived
            alleleholders[1] -= 1;
            alleleholders[2] += 1;
        }
    }
...
void population::mutatedown(int n)
{
    ...
    for (int i=0; i<n; i++)
    {
        if ((alleleholders[2] != 0) &&
            (mt_rand(gent) > (0.5 * alleleholders[1]) / alleleholders[2]))
        {
            // Case 1: mutate a hom-derived → heterozygote
            alleleholders[2] -= 1;
            alleleholders[1] += 1;
        }
        else if (alleleholders[1] != 0)
        {
            // Case 2: mutate a heterozygote → hom-ancestral
            alleleholders[1] -= 1;
            alleleholders[0] += 1;
        }
    }
